...
internal sealed class MediaOptimizationResultConsumer(
    IServiceScopeFactory scopeFactory,
    IOptions<RabbitMqOptions> options,
    IOptions<MessagingOptions> messagingOptions,
    RabbitMqRetryHelper retryHelper,
    ILogger<MediaOptimizationResultConsumer> logger)
    : BackgroundService, IAsyncDisposable
{
    private const string CompletedQueueName = "api.media.optimization.completed";
    private const string CompletedRoutingKey = "media.optimization.completed";
    private const string FailedQueueName = "api.media.optimization.failed";
    private const string FailedRoutingKey = "media.optimization.failed";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "[Media] Optimization result consumer stopped unexpectedly.");
                await CloseAsync();
    ...
    private async Task ConnectAndConsumeAsync(CancellationToken cancellationToken)
    {
        var factory = new ConnectionFactory
        {
            ...
            ClientProvidedName = "kuvox-api-media-optimization-consumer",
        ...
        logger.LogInformation("[Media] Optimization result consumer is ready.");
    }
    ...
    private async Task ConsumeCompletedAsync(CancellationToken cancellationToken)
    {
        ...
        consumer.ReceivedAsync += async (_, ea) =>
            await HandleAsync<MediaOptimizationCompletedEvent>(
                ea,
                CompletedQueueName,
                CompletedRoutingKey,
                (media, completed, ct) => media.HandleOptimizationCompletedAsync(completed, ct));
    ...
    private async Task ConsumeFailedAsync(CancellationToken cancellationToken)
    {
        var consumer = new AsyncEventingBasicConsumer(_channel!);
        consumer.ReceivedAsync += async (_, ea) =>
            await HandleAsync<MediaOptimizationFailedEvent>(
                ea,
                FailedQueueName,
                FailedRoutingKey,
                (media, failed, ct) => media.HandleOptimizationFailedAsync(failed, ct));
        ...
        where T : class
    {
        ...
        try
        {
            ...
            if (message is null || !IsValidMessage(message, expectedEventType))
            {
                await SafeDlqOrRequeueAsync(
                    ea,
                    queueName,
                    new InvalidOperationException("Invalid optimization result message."));
                return;
            }
        ...
        {
            logger.LogWarning(ex, "[Media] Invalid optimization result message.");
            await SafeDlqOrRequeueAsync(ea, queueName, ex);
        }
        catch (DomainException ex)
        {
            logger.LogWarning(ex, "[Media] Rejected optimization result message.");
            await SafeDlqOrRequeueAsync(ea, queueName, ex);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "[Media] Failed to handle optimization result message.");
        ...
        Exception exception)
    {
        ...
        {
            logger.LogError(publishEx, "[Media] Failed to publish optimization result to DLQ.");
        ...
        Exception exception)
    {
        ...
        {
            logger.LogError(publishEx, "[Media] Failed to publish optimization result retry/DLQ.");
            await _channel!.BasicNackAsync(ea.DeliveryTag, multiple: false, requeue: true);
        }
    }
    ...
    private static bool IsValidMessage<T>(T message, string expectedEventType)
        ...
        message switch
        {
            MediaOptimizationCompletedEvent completed =>
                ...
                && completed.Canonical is not null
                && !string.IsNullOrWhiteSpace(completed.Canonical.BucketName)
                && !string.IsNullOrWhiteSpace(completed.Canonical.ObjectKey)
                && !string.IsNullOrWhiteSpace(completed.RawBucketName)
                && !string.IsNullOrWhiteSpace(completed.RawObjectKey),
