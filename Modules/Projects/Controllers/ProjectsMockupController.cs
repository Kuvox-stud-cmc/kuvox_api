...
/// flow (Rule 4) can be exercised end-to-end — watch the logs for the Timelines and Videos
/// cleanup handlers.
/// </summary>
[ApiController]
...
public sealed class ProjectsMockupController(IMediator mediator) : ControllerBase
{
    ...
    [
        new(Guid.Parse("0192a400-0000-7000-8000-000000000001"), SampleOwner, "Summer Travel Vlog", "Beach + roadtrip cut", "draft", DateTimeOffset.UtcNow.AddDays(-3), DateTimeOffset.UtcNow.AddHours(-2)),
        new(Guid.Parse("0192a400-0000-7000-8000-000000000002"), SampleOwner, "Product Launch Teaser", "30s vertical promo", "rendering", DateTimeOffset.UtcNow.AddDays(-1), DateTimeOffset.UtcNow.AddMinutes(-10)),
    ];
    ...
    public ActionResult<ProjectDto> Create(CreateProjectRequest request) =>
        Ok(new ProjectDto(Guid.CreateVersion7(), request.OwnerId, request.Name, request.Description, "draft", DateTimeOffset.UtcNow, DateTimeOffset.UtcNow));

    [HttpDelete("{id:guid}")]
    public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        // Demonstrates Rule 4: publish a cross-module event; Timelines/Videos handlers react.
